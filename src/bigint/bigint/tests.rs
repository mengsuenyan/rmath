...
#[test]
fn bigint_mod_sqrt() {
    let mod_sqrt = |elt: &BigInt, module: &BigInt| {
        let mut sq = elt.sqr();
        sq.rem_euclid_inner(module.clone());
        let sqrt = sq.mod_sqrt(module).unwrap();

        let sq_chk = sq.clone() + module.clone();
        let sqrt_chk = sq_chk.mod_sqrt(module).unwrap();
        assert_eq!(sqrt_chk, sqrt);
        ...
        if sqrt == *elt {
            ...
            let mut sqrt_sq = sqrt.sqr();
            sqrt_sq.rem_euclid_inner(module.clone());
            (sq == sqrt_sq, sqrt)
        }
    };
    ...
    cases.iter().for_each(|&ele| {
        let module = BigInt::from_str(ele).unwrap();
        let modulex4 = module.clone() << 2;
        
        for _ in 1..5 {
            let mut elt = modulex4.random(&mut rng);
            elt -= module.clone();
            let out = mod_sqrt(&elt, &module);
            assert!(out.0, "case: sqrt({}) % {}={}", elt, ele, out.1);
        }
    });
}